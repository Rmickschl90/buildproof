...
export default function DashboardPage() {
  ...
  useEffect(() => {
  (window as any).__runDashboardReconnect = async () => {
    if (isRunningReconnectRef.current) return;
    if (!selectedProjectRef.current?.id) return;

    isRunningReconnectRef.current = true;

    try {
      console.log("🧱 GLOBAL RECONNECT TRIGGER");
      ...
      console.error("Global reconnect failed", error);
    } finally {
