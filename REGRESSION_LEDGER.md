...
## Update (2026-08-04, later same day): mobile pass + rollback plan prepared

**Mobile verification.** Ryan ran a real end-to-end login on Safari on iOS over cellular data - the specific device/browser/network combination flagged above as the biggest coverage gap. Flow: `getleeward.com` -> "Get Started" -> real OTP code received and entered -> `verifyOtp()` succeeded -> landed cleanly on `/dashboard`, no abort error. This exercises the `verifyOtp` catch site specifically (the original trigger site), not just the mount-time redirect. Cross-checked via the newly-connected Vercel MCP: `get_runtime_errors` on the production project (`buildproof-staging`, prj_ZxmifIVb26QcPOnamqThYkctH5lw) showed zero errors over the prior 7 days; runtime logs for the surrounding window (1h retention on the current plan) showed a clean full login sequence with no `login-abort-race` diagnostic hits. Still open: Chrome Android (any network), and the Stripe checkout flow through to the app on a mobile device - explicitly not verified by Ryan in this pass.

**Rollback plan prepared and safety-checked, not yet drilled live.** Using the Vercel MCP (`get_deployment`/`list_deployments`), confirmed the exact deployment currently aliased to `app.getleeward.com`: `dpl_4afFikp3jrwHzzXa7Gx6vPqsMKKc`, commit `1e989f1e449d2f40b2d169be4b6ecc83fe2bf518` ("Add fire-and-forget diagnostics logging for login abort-race recovery path"), `target: production`, `state: READY`, alias list includes `app.getleeward.com` directly (not inferred). This is the anchor commit to redeploy if a future change breaks login - must be updated whenever a newer commit is confirmed good in production, not treated as a permanent pointer.

Checked Vercel's own documentation (`search_vercel_documentation`) rather than assuming: `vercel rollback <deployment-url>` / dashboard "Instant Rollback" is explicitly documented as **a Pro/Enterprise-plan feature** - this project is on Hobby, so that fast path may not actually be available and should not be relied on as the primary method under pressure. The proven, plan-agnostic fallback is the same one used to recover from the 2026-07-19/20 accidental-rollback incident: check out the known-good commit into an isolated git worktree, then `vercel deploy --project buildproof-staging --prod` from that worktree, and confirm the resulting deployment's `meta.gitCommitSha` matches exactly (not just trusting "success" output - the same discipline used when the 4 Team Accounts migrations were verified on production). Full runbook: `09-Regression-Ledger/Production Rollback Runbook.md` in the Brain vault.

Safety-checked without touching production: created an isolated git worktree at commit `1e989f1e` locally (no changes to the working branch), confirmed `isAbortRace`/`reportAbortRace`/`hardNavigateToSigningIn` are present in `app/login/page.tsx` and `app/api/diagnostics/login-abort/route.ts` exists - i.e. this commit really is what we think it is - then removed the worktree. Did not execute an actual production deployment as part of this check; unnecessary since production is already healthy, and production deployment/alias routing is a protected system. One harmless leftover: the worktree's git-internal metadata folder couldn't be force-removed due to a filesystem lock on this session's mounted environment (matches 3 other pre-existing stale worktree entries already in this repo, e.g. from the July incident's own recovery) - cosmetic only, `git worktree prune` from a normal terminal will clear it.
---

# APP REVIEW DEMO-ACCOUNT LOGIN — FIXED — 2026-08-04

## Objective
Apple App Store submission for "Leeward Records" was stuck "Waiting for Review." While Ryan was on hold with Apple support, reviewed the submitted App Review Information Notes and found a real risk before it became an actual rejection: the demo account (`rmickschl23@gmail.com`) was documented as signing in via a real emailed OTP code, but that code goes to Ryan's personal inbox, not anywhere the reviewer can see it - a well-known App Review stall/rejection pattern for passwordless sign-in flows. Fixed proactively, with Ryan's explicit go-ahead to touch `app/login/page.tsx` despite it being a protected/actively-monitored system (same day as the login OTP abort-race fix above).

## Design
`app/login/page.tsx`'s `handleVerifyCode` now always calls a new route, `app/api/auth/review-demo-token/route.ts`, on every verify attempt. The route compares the submitted email/code against two server-only env vars (`APP_REVIEW_DEMO_EMAIL`, `APP_REVIEW_DEMO_CODE`, never exposed to the client) and, on a match, mints a real one-time code via Supabase's admin API (`auth.admin.generateLink` - never sends an email). Any non-match falls straight through to the normal `supabase.auth.verifyOtp()` path, byte-for-byte unchanged. Narrowly scoped by construction to the one designated demo email.

## Two real bugs found getting this working

1. **Client-side gate never fired (first deploy).** An earlier version pre-checked `cleanEmail === process.env.NEXT_PUBLIC_APP_REVIEW_DEMO_EMAIL` in the browser before calling the server route. Pulled the actual deployed JS bundle directly via `fetch()` on the chunk URL and inspected raw bytes/char-codes around the match - confirmed the compiled code still contained a literal, un-inlined `process.env.NEXT_PUBLIC_APP_REVIEW_DEMO_EMAIL` property access, never substituted with the real value despite the env var being added to Vercel beforehand. Root cause of this specific inlining failure was never fully pinned down (Next 16 + Turbopack with a build cache restored from a prior deployment was the leading theory, unconfirmed). Fixed by removing the dependency entirely rather than chasing it further: call the server route unconditionally on every verify attempt, decide match/no-match server-side only. Strictly better than a workaround - also stops the demo email from ever shipping in the public client bundle.

2. **Still 404 in production after that fix (second deploy) - wrong Vercel project.** With the client gate removed, network trace confirmed the route was being hit correctly, but it returned 404 (`{match:false}`) - meaning the server-only env vars weren't visible at all. `vercel env ls production` revealed the actual cause: all three `vercel env add NAME production` commands had been run while the local `.vercel` folder was linked to `leeward-staging-internal` (staging), not `buildproof-staging` (the project that actually serves `app.getleeward.com`). `vercel env add` has no `--project` flag the way `vercel deploy` does - it always targets whatever project is currently linked locally. The earlier `vercel deploy --project buildproof-staging --prod` had been deploying the right code to the right place the whole time; only the env vars landed on the wrong project. Fixed by temporarily relinking (`vercel link` -> "Link to existing project" -> typed `buildproof-staging` explicitly, not accepting any suggested default), re-adding all three vars there, redeploying, then relinking back to `leeward-staging-internal` to restore the normal local default afterward. Same class of mistake as the documented `buildproof-staging`/`buildproof-kappa` naming confusion elsewhere in this repo's history - acting on the wrong Vercel project without an explicit, verified target - just surfaced in `vercel env add` this time instead of `vercel deploy` or git auto-deploy.

## Verification
Performed live against production via Claude in Chrome after the corrected (second) deploy, not just code review. Direct API probes: wrong code for the demo email -> `401 {match:false}`; correct fixed reviewer code for the demo email -> `200 {match:true, token: <real otp>}`; the same correct code against a different, non-demo email -> `401 {match:false}` (confirms narrow scope). Then the full UI flow end-to-end: requested a real code, entered a deliberately wrong code first and got the normal "Token has expired or is invalid" error (confirms the real-code path is completely untouched), then entered the fixed reviewer code and landed cleanly on `/dashboard` with the real, full record list for the account.

## Result
Fixed, deployed to production (`buildproof-staging` project, aliased to `app.getleeward.com`), and behaviorally verified end-to-end - not just at the API layer. Not yet done: the Apple App Store Connect "App Review Information" Notes field still describes the old live-email-wait instructions and needs updating to lead with the fixed reviewer code before resubmitting for review.
