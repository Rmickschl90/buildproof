...
## Checkpoint: share page project-created date timezone fix

Scope:
- share timeline view only
- no PDF changes
- no send/reconnect changes
- no offline changes

Issue:
- share timeline link showed project created date one day ahead near UTC day rollover
- dashboard and PDF outputs were already correct

Fix:
- changed share page project-created display to use timezone-aware formatter with existing proof/approval timezone offset

Result:
- logic fix applied
- natural edge-case verification requires testing a project created within the UTC/local day rollover window