...
export async function remapOfflineProofProjectId(
  ...
  try {
    ...
    const records = await promisifyRequest<OfflineProofRecord[]>(
      index.getAll(oldProjectId)
    );
    ...
    await new Promise<void>((resolve, reject) => {
      tx.oncomplete = () => resolve();
      tx.onerror = () =>
        reject(tx.error ?? new Error("Failed to remap offline proof project ids"));
      tx.onabort = () =>
        reject(tx.error ?? new Error("Offline proof project remap aborted"));
    });
  } finally {
    db.close();
  }
