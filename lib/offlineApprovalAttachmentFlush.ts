...
export async function flushOfflineApprovalAttachmentOutbox(
  ...
  for (const record of records) {
    try {
      ...
      await markOfflineApprovalAttachmentUploading(record.id);
      ...
      if (!insertRes.ok) {
        throw new Error(insertJson?.error || "Approval attachment metadata insert failed");
      }

      await removeOfflineApprovalAttachmentRecord(record.id);

      if (typeof window !== "undefined") {
        window.dispatchEvent(new Event("buildproof-approval-attachment-complete"));
      }
    } catch (err: any) {
